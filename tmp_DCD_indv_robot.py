...
def run_simulation(no_agents, graph_type, graph_size, edge_probability, mu, sigma, clock_rate, simulation_time, parameter_type, switch_off, comm_limit=1):
    ...
    ##
    agent_val = [[] for n in range(no_agents)]
    agent_diff = [[] for n in range(no_agents)]
    agent_steps = [[] for n in range(no_agents)]
    for uuid in range(no_agents):
        agent_val[uuid].append(val[uuid] - mean_0) ##
        agent_diff[uuid].append(sigma)
    ##
    ...
    while True:
        ...
        if len(Gnp.agents[pos]): # i.e. not alone on node
            ...
            ##
            agent_val[uuid].append(avg_val-mean_0)
            agent_diff[uuid].append(abs(val[uuid]-val[neighbors[0]]))
            ##
            ...
            ##
            agent_val[uuid].append(agent_val[uuid][-1])
            agent_diff[uuid].append(agent_diff[uuid][-1])
            ##
    ...
    # or return results
    elif parameter_type == 'oneagent':
        return (agent_val, agent_diff, agent_steps)
